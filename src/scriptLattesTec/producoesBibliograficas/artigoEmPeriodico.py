...
class ArtigoEmPeriodico:
    ...
    def csv(self):
        ...
        s += (self.doi + '\t' if self.doi and self.doi != 'http://dx.doi.org/' else '\t')
        s += (self.autores + '\t' if self.autores else '\t')
        s += (self.titulo + '\t' if self.titulo else '\t')
        s += (self.revista + '\t' if self.revista else '\t')
        s += (self.paginas + '\t' if self.paginas else '\t')
        s += (self.volume + '\t' if self.volume else '\t')
        s += (self.numero + '\t' if self.numero else '\t')
        s += (self.issn + '\t' if self.issn else '\t')
        s += (self.item + '\t' if self.item else '\t')
        s += (self.qualis + '\t' if self.qualis else '\t')
        s += (self.qualissimilar + '\t' if self.qualissimilar else '\t')

        return s

    def colunas_csv(self):
        return 'ANO' + '\t' + 'RELEVANTE' + '\t' + 'DOI' + '\t' + 'AUTORES' + '\t' + 'TITULO' + '\t' + 'REVISTA' \
               + '\t' + 'PAGINAS' + '\t' + 'VOLUME' + '\t' + 'NUMERO' + '\t' + 'ISSN' + '\t' + 'item' + '\t' \
               + 'QUALIS' + '\t' + 'QUALIS-SIMILAR'

    # ------------------------------------------------------------------------ #
    def __str__(self):
        s = "\n[ARTIGO EM PERIODICO] \n"
        s += "+ID-MEMBRO   : " + str(next(iter(self.idMembro))) + "\n"
        s += "+RELEVANTE   : " + str(self.relevante) + "\n"
        s += "+DOI         : " + (self.doi + "\n" if self.doi else "\n")
        s += "+AUTORES     : " + (self.autores + "\n" if self.autores else "\n")
        s += "+AUTORES_C   : " + (self.autores_completo + "\n" if self.autores_completo else "\n")
        s += "+TITULO      : " + (self.titulo + "\n" if self.titulo else "\n")
        s += "+REVISTA     : " + (self.revista + "\n" if self.revista else "\n")
