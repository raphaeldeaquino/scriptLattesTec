...
from scriptLattesTec.qualis import *
...
from .geradorDeArquivosCsv import GeradorDeArquivosCsv
from .geradorDeGrafosCsv import GeradorDeGrafosCsv
...
class Grupo:
    ...
    def __init__(self, arquivo):
        ...
        for linha in fileinput.input(entrada):
            ...
            if linhaDiv[0].strip():
                ...
                linha = linhaDiv[4].strip() if len(linhaDiv) > 4 and linhaDiv[3].strip() else 'membro externo'
                ...
                self.listaDeMembros.append(
                    Membro(idSequencial, identificador, nome, periodo, rotulo, linha, self.itemsDesdeOAno,
                           self.itemsAteOAno, self.diretorioCache, self.dicionarioDeGeolocalizacao))
    ...
    def __str__(self):
        return str(self.listaDeMembros[4])
    ...
    def gerarArquivoJSON(self, nomeArquivo):
        ...
        for membro in self.listaDeMembros:
            nomeCompleto = unicodedata.normalize('NFKD', membro.nomeCompleto).encode('ASCII', 'ignore')
            string += '\n{{ "name":"{0}", "idlattes":"{1}", "rotulo":"{2}", "collaborationrank":{3} }},'.format(
                nomeCompleto, membro.idLattes, membro.rotulo, self.vectorRank[i])
            i += 1
        string = string.strip(',')
        string += '\n],'
        ...
        for i in range(0, N):
            for j in range(i + 1, N):
                if (i != j) and (matriz[i, j] > 0):
                    string += '\n{{ "source":{0}, "target":{1}, "value":{2} }},'.format(str(i), str(j),
                                                                                        str(matriz[i, j]))
        string = string.strip(',')
        string += '\n]\n}'
    ...
    def gerarArquivosCsv(self):
        arquivosCsv = GeradorDeArquivosCsv(self)

    def gerarGrafosCsv(self):
        arquivosCsv = GeradorDeGrafosCsv(self)

    def compilarListasDeItems(self):
        self.compilador = CompiladorDeListas(self)  # compilamos todos e criamos 'listasCompletas'
        ...
        print('comp', self.compilador.listaCompletaArtigoEmPeriodico)
    ...
    def gerarGraficosDeBarras(self):
        ...
        gBarra.criarGrafico(self.compilador.listaCompletaCursoDeCurtaDuracaoMinistrado, 'PT7',
                            'Numero de producoes tecnicas')
        gBarra.criarGrafico(self.compilador.listaCompletaDesenvolvimentoDeMaterialDidaticoOuInstrucional, 'PT8',
                            'Numero de producoes tecnicas')
    ...
    def get_lista_de_membros(self):
        return self.listaDeMembros
