...
class GeradorDeGrafosCsv:
    ...
    def __init__(self, grupo):
        ...
        self.gerarGrafosDeProducoesBibliograficas()
        self.gerarGrafosDeProducoesTecnicas()
    ...
    def gerarGrafosDeProducoesBibliograficas(self):
        self.nPB0 = 0
        self.nPB1 = 0
        self.nPB2 = 0
        self.nPB3 = 0
        ...
        if self.grupo.obterParametro('relatorio-incluir_artigo_em_periodico'):
            self.nPB0 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaArtigoEmPeriodico, "GPB0")
        if self.grupo.obterParametro('relatorio-incluir_livro_publicado'):
            self.nPB1 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaLivroPublicado,
                                                       "GPB1")

        if self.grupo.obterParametro('relatorio-incluir_capitulo_de_livro_publicado'):
            self.nPB2 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaCapituloDeLivroPublicado,
                                                       "GPB2")
        if self.grupo.obterParametro('relatorio-incluir_texto_em_jornal_de_noticia'):
            self.nPB3 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaTextoEmJornalDeNoticia,
                                                       "GPB3")
        if self.grupo.obterParametro('relatorio-incluir_trabalho_completo_em_congresso'):
            self.nPB4 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaTrabalhoCompletoEmCongresso,
                                                       "GPB4")
        if self.grupo.obterParametro('relatorio-incluir_resumo_expandido_em_congresso'):
            self.nPB5 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaResumoExpandidoEmCongresso,
                                                       "GPB5")
        if self.grupo.obterParametro('relatorio-incluir_resumo_em_congresso'):
            self.nPB6 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaResumoEmCongresso,
                                                       "GPB6")
        if self.grupo.obterParametro('relatorio-incluir_artigo_aceito_para_publicacao'):
            self.nPB7 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaArtigoAceito,
                                                       "GPB7")
        #if self.grupo.obterParametro('relatorio-incluir_apresentacao_de_trabalho'):
        #    self.nPB8 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaApresentacaoDeTrabalho,
        #                                               "GPB8")
        if self.grupo.obterParametro('relatorio-incluir_outro_tipo_de_producao_bibliografica'):
            self.nPB9 = self.gerar_grafo_de_producoes(
                self.grupo.compilador.listaCompletaOutroTipoDeProducaoBibliografica, "GPB9")
        # Total de produção bibliográfica
        self.nPB = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaPB,
                                                  "GPB")


    def gerarGrafosDeProducoesTecnicas(self):
        self.nPT0 = 0
        self.nPT1 = 0
        self.nPT2 = 0
        self.nPT3 = 0
        ...
        if self.grupo.obterParametro('relatorio-incluir_software_com_patente'):
            self.nPT0 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaSoftwareComPatente,
                                                       "GPT0")
        if self.grupo.obterParametro('relatorio-incluir_software_sem_patente'):
            self.nPT1 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaSoftwareSemPatente,
                                                       "GPT1")
        if self.grupo.obterParametro('relatorio-incluir_produto_tecnologico'):
            self.nPT2 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaProdutoTecnologico,
                                                       "GPT2")
        if self.grupo.obterParametro('relatorio-incluir_processo_ou_tecnica'):
            self.nPT3 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaProcessoOuTecnica,
                                                       "GPT3")
        if self.grupo.obterParametro('relatorio-incluir_trabalho_tecnico'):
            self.nPT4 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaTrabalhoTecnico,
                                                       "GPT4")
        if self.grupo.obterParametro('relatorio-incluir_outro_tipo_de_producao_tecnica'):
            self.nPT5 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaOutroTipoDeProducaoTecnica,
                                                       "GPT5")
        if self.grupo.obterParametro('relatorio-incluir_apresentacao_de_trabalho'):
            self.nPT6 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaApresentacaoDeTrabalho,
                                                       "GPT6")
        if self.grupo.obterParametro('relatorio-incluir_curso_de_curta_duracao_ministrado'):
            self.nPT7 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaCursoDeCurtaDuracaoMinistrado,
                                                       "GPT7")
        if self.grupo.obterParametro('relatorio-incluir_desenvolvimento_de_material_didatico_ou_instrucional'):
            self.nPT8 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaDesenvolvimentoDeMaterialDidaticoOuInstrucional,
                                                       "GPT8")
        if self.grupo.obterParametro('relatorio-incluir_organizacao_de_evento'):
            self.nPT9 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaOrganizacaoDeEvento,
                                                       "GPT9")
        if self.grupo.obterParametro('relatorio-incluir_programa_de_radio_ou_tv'):
            self.nPT10 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaProgramaDeRadioOuTv,
                                                       "GPT10")
        if self.grupo.obterParametro('relatorio-incluir_relatorio_de_pesquisa'):
            self.nPT11 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaRelatorioDePesquisa,
                                                       "GPT11")
        if self.grupo.obterParametro('relatorio-incluir_carta-mapa-ou-similar'):
            self.nPT12 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaCartaMapaOuSimilar,
                                                       "GPT12")
        if self.grupo.obterParametro('relatorio-incluir_patente'):
            self.nPT13 = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaPatente,
                                                       "GPT13")
        # Total de produções técnicas
        self.nPT = self.gerar_grafo_de_producoes(self.grupo.compilador.listaCompletaPT, "PT")
    ...
    def gerar_grafo_de_producoes(self, lista_completa, prefixo):
        ...
        if keys:  # apenas geramos páginas web para lista com pelo menos 1 elemento
            ...
            membros = list(map(lambda m: m.nomeCompleto, self.grupo.listaDeMembros))
            membros = list(map(
                lambda m: ''.join(c for c in unicodedata.normalize('NFD', m.lower()) if unicodedata.category(c) != 'Mn'),
                membros))
            linhas = list(map(lambda m: m.linha, self.grupo.listaDeMembros))
            graph = Graph()

            for ano, indice_no_ano, publicacao in anos_indices_publicacoes:
                autores = publicacao.autores_completo.split('; ')
                for i in range(len(autores)):
                    for j in range(i + 1, len(autores)):
                        autor_name1 = ''.join(c for c in unicodedata.normalize('NFD', autores[i].lower()) if unicodedata.category(c) != 'Mn')
                        if ', ' in autor_name1:
                            autor_name1 = autor_name1.split(', ')
                            autor_name1 = autor_name1[1] + ' ' + autor_name1[0]
                        if '. ' in autor_name1:
                            tokens_a = autor_name1.split(' ')
                            for m in membros:
                                tokens_b = m.split(' ')
                                if len(tokens_a) == len(tokens_b):
                                    igual = True
                                    for t in range(len(tokens_a) - 1):
                                        if tokens_a[t][0] != tokens_b[t][0]:
                                            igual = False
                                    if tokens_a[len(tokens_a) - 1] != tokens_b[len(tokens_b) - 1]:
                                        igual = False
                                    if igual:
                                        autor_name1 = m
                                        break
                        linha1 = None
                        try:
                            if 'zang' in autor_name1 and 'fonseca' in autor_name1:
                                autor_name1 = 'warde antonieta da fonseca zang'
                            index = list(membros).index(autor_name1)
                            linha1 = linhas[index]
                        except ValueError:
                            linha1 = 'Membro Externo'
                        autor_name2 = ''.join(c for c in unicodedata.normalize('NFD', autores[j].lower()) if unicodedata.category(c) != 'Mn')
                        if ', ' in autor_name2:
                            autor_name2 = autor_name2.split(', ')
                            autor_name2 = autor_name2[1] + ' ' + autor_name2[0]
                        if '. ' in autor_name2:
                            tokens_a = autor_name2.split(' ')
                            for m in membros:
                                tokens_b = m.split(' ')
                                if len(tokens_a) == len(tokens_b):
                                    igual = True
                        ...
                        linha2 = None
                        try:
                            if 'zang' in autor_name2 and 'fonseca' in autor_name2:
                                autor_name2 = 'warde antonieta da fonseca zang'
                            index = list(membros).index(autor_name2)
                            linha2 = linhas[index]
                        except ValueError:
                            linha2 = 'Membro Externo'
                        graph.add_edge({'name': autor_name1, 'line': linha1}, {'name': autor_name2, 'line': linha2})

            producoes_csv = graph.to_csv()
